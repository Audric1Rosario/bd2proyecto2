...
public class ListaPremios extends JDialog {

	private final JPanel contentPanel = new JPanel();

	// Tablas
	private static JTable tableListaPremios;
	...
	public static void main(String[] args) {
		try {
			ListaPremios dialog = new ListaPremios();
	 ...
	 * Create the dialog.
	 */
	public ListaPremios(/* Empleado usuarioActual */) {
		// this.usuarioActual = usuarioActual;
		setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
		setResizable(false);
		setTitle("Control de usuarios.");
		setIconImage(Toolkit.getDefaultToolkit().getImage(ListaPremios.class.getResource("/image/game-controller.png")));
		...
		String[] headers = { "Nombre", "Apellido", "Usuario", "Correo", "Puntuación" };
		model = new DefaultTableModel();
		model.setColumnIdentifiers(headers);
		tableListaPremios = new JTable() {
			public boolean isCellEditable(int rowIndex, int vColIndex) {
				return false;
			}
		};
		tableListaPremios.setModel(model);
		tableListaPremios.getTableHeader().setResizingAllowed(false);
		tableListaPremios.getTableHeader().setReorderingAllowed(false);
		/*
		 * tableListaPremios.getSelectionModel().addListSelectionListener(new
		 * ListSelectionListener(){ public void valueChanged(ListSelectionEvent event) {
		 * if (tableListaPremios.getSelectedRow() != -1) { enfermedadModificar =
		 * Clinica.getInstance().buscarEnfermedadByNombre(tableListaPremios.getValueAt(
		 * tableListaPremios.getSelectedRow(), 0).toString()); rellenarDatos();
		 * 
		 ...
		 */
		scrollPane.setViewportView(tableListaPremios);
		{
			...
			JLabel lblPuntuacion = new JLabel("Puntuaci\u00F3n Requerida");
			lblPuntuacion.setBounds(10, 79, 232, 14);
			panel_1.add(lblPuntuacion);

			JLabel lblTicket = new JLabel("Tickets Requeridos");
			lblTicket.setBounds(10, 137, 232, 14);
			panel_1.add(lblTicket);

			JLabel lblLimite = new JLabel("L\u00EDmite");
			lblLimite.setBounds(10, 195, 232, 14);
			panel_1.add(lblLimite);
			
			JSpinner spnTicket = new JSpinner();
			spnTicket.setEnabled(false);
			spnTicket.setBounds(10, 162, 232, 20);
			panel_1.add(spnTicket);
			
			JSpinner spnPuntuacion = new JSpinner();
			spnPuntuacion.setEnabled(false);
			spnPuntuacion.setBounds(10, 104, 232, 20);
			panel_1.add(spnPuntuacion);
			
			JSpinner spnLimite = new JSpinner();
			spnLimite.setEnabled(false);
			spnLimite.setBounds(10, 220, 232, 20);
			panel_1.add(spnLimite);
		}
		{
			JPanel panel_1 = new JPanel();
			 ...
			 * actionPerformed(ActionEvent e) { clear(); tableListaPremios.clearSelection();
			 * rellenarTabla(txtBuscar.getText()); } });
			 */
			btnBuscar.setIcon(new ImageIcon(ListaPremios.class.getResource("/image/magnifying-glass.png")));
			 ...
			 * rellenarTabla(""); } });
			 */
			btnRefresh.setIcon(new ImageIcon(ListaPremios.class.getResource("/image/reload.png")));
